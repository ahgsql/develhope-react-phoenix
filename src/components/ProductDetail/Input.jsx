...
const Input = ({ id, setComments, oldComments }) => {
      ...
      <div id="InputRow">
        <div id="Input">
          <label htmlFor="comment">
            <h2 style={{ textAlign: "start" }}>Enter Your Comment</h2>
          </label>
          <textarea
            id="comment"
            ...
            disabled={!user}
            placeholder={
              user ? "Enter your comment" : "Please Sign In to Comment"
            }
            value={comment}
            onChange={handleInputChange}
          />
        </div>
        <button disabled={!user} onClick={handleFormSubmit}>
          Submit
        </button>
      </div>
    </div>
